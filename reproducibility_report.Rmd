...
#pdf("figures/a_bp_top10_nohla.pdf", width = 10, height = 7)
ggplot(a_bp_top10, aes(x=factor(term_name, levels = rev(levels(factor(term_name)))),
                       y=-log10(as.numeric(p_value)), fill="#92278F")) +
    geom_bar(stat="identity") +
    theme_classic() +
...
f_bp <- f_gos.df[grep('GO:BP', f_gos.df[, "source"]), ]
f_bp_top10 <- as.data.frame(f_bp[1:8,])

#pdf("figures/f_bp_top10_nohla.pdf", width = 10, height = 7)
ggplot(f_bp_top10, aes(x=factor(term_name, levels = rev(levels(factor(term_name)))),
                       y=-log10(as.numeric(p_value)), fill="#009444")) +
    geom_bar(stat="identity") +
    theme_classic() +
...
f_final <- distinct(f_csppi.df)
#write.table(f_final, file = "results/string/fetal_CSPPI_400.txt", sep = "\t", col.names = FALSE, row.names=FALSE)
f_final <- read.table(gzfile("results/string/fetal_CSPPI_400.txt"), header = FALSE, sep = "\t")

a_csppi.df <- data.frame()
for(a in 1:nrow(a_PPI_400)){
    if((a_PPI_400[a,][1] %in% names(a_dict)) & (a_PPI_400[a,][2] %in% names(a_dict))){
        p1 <- a_PPI_400[a,][1]; p2 <- a_PPI_400[a,][2]
        t <- c(a_dict[p1$V1], a_dict[p2$V2])
        a_csppi.df <- rbind(a_csppi.df, t)
    }
}
a_final <- distinct(a_csppi.df)
#write.table(a_final, file = "results/string/adult_CSPPI_400.txt", sep = "\t", col.names = FALSE, row.names=FALSE)
a_final <- read.table(gzfile("results/string/adult_CSPPI_400.txt"), header = FALSE, sep = "\t")
```
