...
pacman::p_load(ggplot2, pander, dplyr, tidyr, regioneR, rtracklayer, reshape2, RColorBrewer,
               ComplexHeatmap, circlize, EnsDb.Hsapiens.v86, gprofiler2, igraph)
...
colnames(f_csppi.df) <- c("p1", "p2")
#write.table(f_csppi.df, file = "results/string/fetal_CSPPI_400.txt", sep = "\t", col.names = TRUE, row.names=FALSE)
f_csppi.df <- read.table(gzfile("results/string/fetal_CSPPI_400.txt.gz"), header = TRUE, sep = "\t")
...
colnames(a_csppi.df) <- c("p1", "p2")
#write.table(a_csppi.df, file = "results/string/adult_CSPPI_400.txt", sep = "\t", col.names = TRUE, row.names=FALSE)
a_csppi.df <- read.table(gzfile("results/string/adult_CSPPI_400.txt.gz"), header = TRUE, sep = "\t")
```

We extracted only ASD-associated genes from the fetal and adult CSPPIs to get ASD-specific CSPPIs.
...
#write.table(f_asd_csppi.df, file = "results/string/fetal_asd_CSPPI_400.txt", sep = "\t", col.names = TRUE, row.names=FALSE)
f_asd_csppi.df <- read.table(gzfile("results/string/fetal_asd_CSPPI_400.txt.gz"), header = TRUE, sep = "\t")
f_asd_unique_proteins <- unique(c(f_asd_csppi.df$p1, f_asd_csppi.df$p2)) # ?

a_asd_csppi.df <- subset(a_csppi.df, (a_csppi.df$p1 %in% adult_egenes & a_csppi.df$p2 %in% adult_egenes))
#write.table(f_asd_csppi.df, file = "results/string/fetal_asd_CSPPI_400.txt", sep = "\t", col.names = TRUE, row.names=FALSE)
a_asd_csppi.df <- read.table(gzfile("results/string/adult_asd_CSPPI_400.txt.gz"), header = TRUE, sep = "\t")
a_asd_unique_proteins <- unique(c(a_asd_csppi.df$p1, a_asd_csppi.df$p2)) # ?
```
...
a_graph <- graph_from_data_frame(d=asd_a_links, vertices=asd_a_nodes, directed=F); a_graph
...
f_graph <- graph_from_data_frame(d=asd_f_links, vertices=asd_f_nodes, directed=F); f_graph
...
a_1_path <- query_kegg(a_1)
#a_1_paths <- a_1_path$result
#a_1.df <- apply(a_1_paths, 2, as.character)
#write.table(a_1.df, file = "results/paths/asd_a_CSPPI_cluster1_kegg_fdr.txt", sep = "\t", col.names = TRUE, row.names=FALSE)

communities(a_lc)[2] # to get gene names
a_2 <- communities(aa_lc)$'2' # to get gene names
...
a_3_path <- query_kegg(a_3)
#a_3_paths <- a_3_path$result
#a_3.df <- apply(a_3_paths, 2, as.character)
#write.table(a_3.df, file = "results/paths/asd_a_CSPPI_cluster3_kegg_fdr.txt", sep = "\t", col.names = TRUE, row.names=FALSE)
